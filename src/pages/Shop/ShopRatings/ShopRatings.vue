<template>
  <div class="ratings">
    <div class="ratings-content">
      ...
      <RatingsFilter :selectTypes="selectTypes" :onlyContent="onlyContent"
...
<script>
  ...
  export default {
    data(){
      return {
        selectTypes: 2, // 评论的类型   0: 满意  1: 不满意  2: 全部
        onlyContent: true // 评论是否有内容
      }
    },
    mounted(){
    ...
    computed: {
      ...
      // 需要显示的过滤后的评论列表
      filterRatings(){
        const {ratings,selectTypes,onlyContent} = this
        return ratings.filter(rating => {
          const {rateType,text} = rating
          return (selectTypes === 2 || selectTypes === rateType) && (!onlyContent || text.length>0)
        })
      }
    },
    methods: {
      setSelectTypes(SelectType){
